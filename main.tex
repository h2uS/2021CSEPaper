...
[,circle,fill, 
  ...
  [$\cdots$, before drawing tree={y+=0.5em}, edge={dashed}]
...
Corollary \ref{cor0701}이 의미하는 것은, 특정 확률분포를 구할 수 있는 시행 규칙들 중 가장 시행 횟수 기댓값이 작은 시행 규칙을 찾는 경우에 Theorem \ref{thm02}의 과정에서 $h$가 최소인 경우 하나만 확인해보면 충분하다는 것이다. 예를 들어 Table \ref{tab05}에서 $c=3$일 때는 $h=5$인 경우 하나만 확인하면 충분하다.

가장 시행 횟수 기댓값이 작은 시행 규칙을 찾는 경우에 대해 한 가지를 더 살펴보자. Theorem \ref{thm02}의 과정에서 $z_{1}, \cdots, z_{n}$가 모두 $k$의 배수인 경우, 만들어지는 시행 규칙의 모든 leaf 노드를 $k$개씩 모아 노드 합치기를 수행할 수 있다. 이를 수행하고 나면 각 대응값에 대응된 노드 수가 전부 $\displaystyle \frac{1}{k}$배이고 높이가 1 감소한 새로운 포화 $k$진 트리 시행 규칙을 얻을 수 있는데, 이 시행 규칙 또한 Theorem \ref{thm02}의 과정을 통해 얻을 수 있는 시행 규칙이다. 만약 Table \ref{tab05}에서와 같은 방식으로 $c$의 값을 1부터 차례대로 증가시켜가며 Theorem \ref{thm02}를 적용한다고 하면, $c$의 값이 $k$의 배수 $c_{0}$인 경우의 시행 규칙은 $c$의 값이 $\displaystyle \frac{c_{0}}{k}$일 때 이미 구해진 시행 규칙이 된다. Figure \ref{fig09}을 보면 Table \ref{tab05}에서 $c=2, h=4$인 경우 구해지는 시행 규칙에 노드 합치기를 수행한 결과 $c=1, h=3$인 경우 구해지는 시행 규칙인 Figure \ref{fig04}와 같아지는 것을 확인할 수 있다.\\
...
지금까지 특정 확률분포를 구할 수 있는 시행 규칙들 중 가장 시행 횟수 기댓값이 작은 시행 규칙을 찾기 위해 어떤 경우들을 조사하여야 하는지 살펴보았다. 해당 경우들을 차례대로 조사하는 프로그램을 제작하고 보드게임 창작자가 이를 사용한다면, 창작자는 자신이 원하는 확률적 요소를 특정 도구로 구현하는 가장 좋은 방법을 빠르고 간편하게 알아낼 수 있을 것이다.
