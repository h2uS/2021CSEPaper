...
Theorem \ref{thm02}의 과정을 따르면 이산확률변수가 유한한 값을 가지며 각 값을 가질 확률이 모두 유리수인 확률분포가 주어졌을 때, 해당 확률분포를 구할 수 있는 시행 규칙을 만들 수 있다. 그런데 이 과정에서 중간에 $z_{1}, \cdots, z_{n}$, 그리고 $h$를 어떤 값을 취하느냐에 따라서 만들어지는 시행 규칙의 형태가 달라진다. 따라서 이렇게 만들어지는 모든 시행 규칙들 각각에 대해 시행 횟수 기댓값을 줄이는 과정을 최대한 수행하고, 최종적으로 구해지는 시행 횟수 기댓값들을 서로 비교하여 그 중 가장 값이 작은 경우의 $z_{1}, \cdots, z_{n}, h$ 값 조합을 찾는 과정을 생각해볼 수 있다. 이것은 다시 말해 특정 확률분포를 구할 수 있는 시행 규칙들 중 가장 시행 횟수 기댓값이 작은 시행 규칙을 찾는 과정이다.
...
\begin{proof}
$T$가 $l$개의 leaf 노드를 가진 $k$진 트리라고 하자. 각각의 leaf 노드에 대해 $T$의 root에서 시작하여 맨 처음에 도달할 확률, 깊이를 $p_{i}, d_{i}\, (i=1,\cdots,l)$라 하고, leaf 노드들 중 Re가 아닌 노드에 대해 $p_{i}$ 값의 합을 $P$라고 하자. $P$는 $T$의 root에서 시작하여 맨 처음 도달하는 노드가 Re가 아닌 노드일 확률로, Theorem \ref{thm04}의 식의 분모와 동일하다.

Theorem \ref{thm02}의 과정에서 특정 $z_{1}, \cdots, z_{n}, h$ 조합에 대해 $h$만 $h+1$로 증가시킨다는 것은 Theorem \ref{thm02}의 과정이 끝난 직후 만들어진 시행 규칙의 leaf 노드의 수가 $k^{h}$개에서 $k^{h+1}$개로, $(k-1)k^{h}$개 늘어난다는 의미이다. 이때 늘어난 leaf 노드들은 모두 깊이가 $h+1$이고 Re에 대응되므로, 시행 규칙의 시행 횟수 기댓값을 줄이는 과정을 최대한 수행하고 나면 깊이가 1인 $k-1$개의 Re 노드가 된다. 결론적으로 $T'$는 $T$의 root에 형제 노드로 $k-1$개의 Re 노드를 추가하고 새로운 부모 노드를 붙여서 만든 시행 규칙이다. 이를 그림으로 나타내면 다음과 같다.
...
[,circle,fill
  [,circle,fill, edge={->, >=latex}, edge label={node[pos=0.3pt,left,font=\footnotesize]{1}}]
  [$\cdots$, edge={draw=none}]
  [,circle,fill, edge={->, >=latex}, edge label={node[pos=0.3pt,right,font=\footnotesize]{$k-1$}}]
  [,circle,fill, edge={->, >=latex}, edge label={node[pos=0.3pt,right,font=\footnotesize]{$k$}}]
]
\end{forest}
\caption{}
\label{fig08}
