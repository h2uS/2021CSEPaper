...
우선 시행 규칙의 출력은 생략하고 시행 횟수 기댓값만 모든 경우에 대해 출력한 뒤 해당 값이 최소인 순간의 정보를 확인할 수 있는 프로그램을 제작하였다. 최종적으로 구현할 프로그램에선 시행 횟수 기댓값을 모든 경우에 대해 출력할 필요가 없지만, 정상적으로 계산이 이루어지는지 확인하고 또 특정 추측을 검증하기 위해 우선 모든 경우에 대해 출력하도록 하였다. 추측의 내용에 관해서는 Discussion에서 다룬다.

사용할 시행 도구의 결괏값을 나타내는 이산확률변수가 $\textnormal{unif}\{1, k\}$를 따른다고 할 때, 해당 $k$ 값을 프로그램의 입력값으로 두었다. 또한 확률분포를 구성하는 확률들의 정수비도 vector 형태의 입력값으로 두었으며, 입력값으로 주어진 해당 정수비를 이루는 숫자들이 서로소가 아닌 경우 최대공약수로 나누어 서로소가 되도록 변환한 뒤 입력값으로 받아들이도록 하였다.

프로그램은 \ref{subsection3-5}에서 다루었던 것과 같이 정수비를 구성하는 숫자들에 곱할 $c$ 값을 $k$의 배수인 경우는 제외하고 차례대로 증가시켜가며 최종적으로 구해지는 정 $k$진 트리 시행 규칙의 시행 횟수 기댓값을 계산한다. 시행 횟수 기댓값의 계산은 \ref{subsection3-6}에서 다루었던 내용을 기반으로 $c$가 곱해진 확률들의 정수비를 구성하는 숫자들을 각각 $k$진수로 바꾸는 과정에서 바로 실시하며, 시행 규칙을 출력할 필요가 없기 때문에 해당 $k$진수들을 따로 저장하지는 않는다.
...
실제 프로그램의 출력이 어떤 식으로 이루어지는지 살펴보자. 다음은 Table \ref{tab05}에서 다루었던 경우를 프로그램에 입력하여 나온 출력 결과이다. 입력되는 $k$ 값은 2, 정수비 vector는 \{ 1, 2, 3 \}이며 extra depth limit은 5이다.
...
\noindent \ref{subsection3-6}에서의 계산 결과와 동일하게, $c = 5$, $\textnormal{height} = 5$ 일 때 $\textnormal{\textbf{E}[X]} = 3$이 나오는 것을 확인할 수 있다.
\newpage

다음은 입력되는 $k$ 값을 3, 정수비 vector를 \{ 5, 2 \}, extra depth limit을 3으로 설정한 결과 나온 출력 결과이다.
...
\subsection{Printer}
최종적으로 사용 가능한 프로그램을 완성하기 위해, 앞서 만들었던 프로그램을 수정하였다. 우선 사용자가 프로그램 실행 후 $k$ 값, 확률 정수비 vector 구성 숫자, extra depth limit을 직접 입력할 수 있도록 하였고, 시행 횟수 기댓값을 모든 경우에 대해 출력하지 않도록 변경하였다. 각 height 값에서 시행 규칙의 시행 횟수 기댓값이 최소인 경우의 해당 값은 여전히 출력하여 사용자가 확인할 수 있도록 하였고, 시행 규칙은 사전식 나열 형태로 출력하여 txt 파일 형태로 저장하도록 하였다.

txt 파일 내에서 출력되는 시행 규칙은 각 height 값에서 시행 횟수 기댓값이 최소인 시행 규칙이며, height가 더 작으면서 시행 횟수 기댓값이 작거나 같은 시행 규칙이 존재할 경우에는 출력을 생략하도록 하였다. 이는 사용자가 프로그램을 사용하여 구한 시행 규칙을 실제 보드게임에서 사용가능한 형태로 인쇄할 경우, 높이가 큰 시행 규칙일 수록 복잡한 형태가 되므로 가능한 한 높이가 작은 시행 규칙을 사용하려고 할 것이라는 전제를 반영한 것이다.

시행 규칙 출력을 위해 시행 규칙을 나타내는 클래스를 정의하였다. 해당 클래스는 $k$진 트리의 노드를 나타내는 구조체를 사용하며, $c$가 곱해진 확률들의 정수비를 구성하는 숫자들을 $k$진수로 바꾼 결과를 2차원 vector로 저장하고 이를 이용하여 시행 규칙 내용을 완성하는 생성자를 가진다. 이 과정은 \ref{subsection3-6}에서 설명한 과정과 동일하다.

클래스 내에 시행 규칙을 사전식 나열 형태로 출력하기 위한 함수도 정의하였으며, 해당 함수는 전위 순회(preorder traveresal)를 root 노드로부터의 경로를 기억하고 노드 값과 함께 출력하는 방식으로 구현한 것이다.

다음은 $k$ 값을 3, 정수비 vector를 \{ 5, 2 \}, extra depth limit을 6으로 입력하여 나온 터미널 출력 결과이다.
...
(Ex. 동전 2 / 주사위 6)
k 값 입력 : 3
각각의 사건이 일어날 확률들의 정수비를 구성하는 숫자를 차례대로 입력해주세요.
...
\noindent이후 출력 파일이 생성되며 파일의 내용은 다음과 같다.
